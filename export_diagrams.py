...
def main():
    ...
    if os.path.exists(USECASES_DIR):
        for role in ["ADMIN", "MANAGER", "STAFF"]:
            role_dir = os.path.join(USECASES_DIR, role)
            if os.path.exists(role_dir):
                role_out_dir = os.path.join(OUTPUT_USECASES_DIR, role)
                os.makedirs(role_out_dir, exist_ok=True)
                for file in os.listdir(role_dir):
                    if file.endswith(".drawio"):
                        input_file = os.path.join(role_dir, file)
                        output_name = file[:-7] + ".png"
                        output_file = os.path.join(role_out_dir, output_name)
                        tasks.append((input_file, output_file, f"UseCase - {role}"))
